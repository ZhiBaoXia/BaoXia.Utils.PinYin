using BaoXia.Utils.PinYin.Utils;
using System;
...
{
        public class ChinesePinYinInfesCodeCreator
        {
                ////////////////////////////////////////////////
                // @类方法
                ////////////////////////////////////////////////
                ...
                public static void CreateChinesePinYinInfoCodeFiles()
                {
                                ...
                                objectPinYinIndex++)
                        {
                                ...
                                foreach (var chinesePinYinInfo in chinesePinYinInfes)
                                {
                                        ...
                                        Console.WriteLine(
                                                ...
                                                objectPinYinIndex + 1,
                                                100.0 * progressIndex / chinesePinYinInfes.Length);
                                }
                                ...
                                {
                                        var compareValue = CharsComparator.CompareChars(
                                ...
                                // {
                                //         return CharsComparator.CompareChars(
                                //                 pinYinA.ToCharArray(),
                                //                 pinYinB.ToCharArray());
                                // });
                ...
                public static void CreateChineseInMultipleCharsCodeFile()
                {
                        var chineseCharactersInMultipleChars = new List<string>();
                        var chinesePinYinOriginalInfes = ChinesePinYinOriginalInfes.Infes;
                        foreach (var chinesePinYinOriginalInfo in chinesePinYinOriginalInfes)
                        {
                                var chineseCharacter = chinesePinYinOriginalInfo.ChineseCharacter;
                                if (chineseCharacter?.Length > 1)
                                {
                                        chineseCharactersInMultipleChars.Add(chineseCharacter);
                                }
                        }
                        chineseCharactersInMultipleChars.Sort(
                                (chineseCharacterA, chineseCharacterB) =>
                                {
                                        return CharsComparator.CompareChars(
                                                chineseCharacterA.ToCharArray(),
                                                chineseCharacterA.ToCharArray());
                                });

                        var chineseCharacterInMultipleCharsAnchorChars = new List<string>();
                        string lastChineseCharacter = string.Empty;
                        foreach (var chineseCharacter in chineseCharactersInMultipleChars)
                        {
                                if (chineseCharacterInMultipleCharsAnchorChars.Count < 1)
                                {
                                        chineseCharacterInMultipleCharsAnchorChars.Add(chineseCharacter);
                                }
                                else
                                {
                                        var isCharsContinuous = false;
                                        if (lastChineseCharacter.Length == chineseCharacter.Length)
                                        {
                                                isCharsContinuous = true;
                                                for (var charIndex = 0;
                                                        charIndex < lastChineseCharacter.Length;
                                                        charIndex++)
                                                {
                                                        if ((charIndex + 1) == lastChineseCharacter.Length)
                                                        {
                                                                if ((lastChineseCharacter[charIndex] + 1) != chineseCharacter[charIndex])
                                                                {
                                                                        isCharsContinuous = false;
                                                                        break;
                                                                }
                                                        }
                                                        else if (lastChineseCharacter[charIndex] != chineseCharacter[charIndex])
                                                        {
                                                                isCharsContinuous = false;
                                                                break;
                                                        }
                                        ...
                                        if (isCharsContinuous != true)
                                        {
                                                chineseCharacterInMultipleCharsAnchorChars.Add(chineseCharacter);
                                        }
                                }
                                // !!!
                                lastChineseCharacter = chineseCharacter;
                                // !!!
                        }
                        if (chineseCharacterInMultipleCharsAnchorChars.Last()
                                != chineseCharactersInMultipleChars.Last())
                        {
                                chineseCharacterInMultipleCharsAnchorChars.Add(chineseCharactersInMultipleChars.Last());
                        }
                        ...
                        //        + "namespace BaoXia.Utils.PinYin\r\n"
                        //        + "{\r\n"
                        //        + "\tpublic class CreateChineseInMultipleChars\r\n"
                        //        + "\t{\r\n"
                        //        + "\r\n"
                        //        + charsEndSymbolCodes + "\r\n"
                        //        + "\r\n"
